...
def get_plotting_attributes(varname):
    ...
    attrs = PLOTTING_ATTRIBUTES[varname].copy()

    # Handle cmap_params → actual colormap
    if "cmap_params" in attrs:
        attrs["cmap"] = modified_colormap(*attrs.pop("cmap_params"))

    # Handle d_cmap_params → actual colormap
    if "d_cmap_params" in attrs:
        attrs["d_cmap"] = modified_colormap(*attrs.pop("d_cmap_params"))

    # Handle centered norms
    if "norm_center" in attrs:
        attrs["norm"] = mcolors.CenteredNorm(vcenter=attrs.pop("norm_center"))
...
def multi_experiment_lat_lon(
    ...
    plotting_attributes = get_plotting_attributes(variable_data.name)
    ...
    for ax, experiment in zip(axes, coords.experiments.values):
        ax.set_title(f"{EXPERIMENT_DISPLAY_NAMES[experiment]}")
...
def multi_experiment_lon_height(
    ...
    plottign_attributes = get_plotting_attributes(variable_data.name)
