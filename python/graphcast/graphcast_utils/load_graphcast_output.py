...
def load_predictions(prediction_initial_date, prediction_timesteps, prediction_type, run_notes, vars_to_load, decode_time=True):
    ...
    predictions = xr.open_zarr(prediction_filepath, drop_variables=vars_to_drop).sel(batch=0, drop=True)

    if decode_time:
        datetimes = convert_ns_to_datetime(predictions.time, prediction_initial_date)
        predictions = predictions.assign_coords({'time':datetimes.datetime}).drop_vars(["datetime", "initial_conditions_path"])
