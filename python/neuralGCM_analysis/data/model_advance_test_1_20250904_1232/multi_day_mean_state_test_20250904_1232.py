...
#    corrected_initial_state = corrected_initial_state.replace(memory = final_state.memory - mean_state_correction_memory)
    ...
    final_state = model.advance(
        corrected_initial_state,
        input_forcings
    )
    new_predictions_list.append(model.data_to_xarray(model.decode(final_state, input_forcings), times=None))
...
experiment_parameters = {
    'experiment_name': 'model_advance_test_1',
    'description': 'This test uses the 2.8 deg model and 1-hr timesteps along with the model.advance function',
