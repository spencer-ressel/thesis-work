...
data_inner_steps = 12  # process every 24th hour
...
eval_era5 = xarray.load_dataset(
    f"../neuralgcm/neuralgcm/input/thirty_year_DJF_climatology.nc"
).isel(time=0, drop=True).expand_dims(time=[pd.Timestamp('2000-01-15')])
print('\u2713')

#############  MODEL
print(f"{'Configuring model...':<30}", end="")
inner_steps = 12  # save model outputs at 'inner steps' intervals
outer_steps = 1 * 12 // inner_steps  # total of 'outer steps' days
...
experiment_parameters = {
    'experiment_name': 'climatology_test_2',
    'description': 'This test uses a 30-year DJF climatology as the mean-state and runs with a 12-hr timestep.',
